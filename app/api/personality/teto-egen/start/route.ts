...
export const POST = withErrorHandling('personality/teto-egen/start', async (req: Request) => {
  const bypass =
    isRateLimitDisabled() ||
  ...
  if (!isSupabaseConfigured()) {
    ...
    return NextResponse.json({
      ...
      questions: getTetoEgenQuestions(locale),
    });
  }
  ...
  const { data, error } = await admin
    .from('test_sessions')
    .insert({ locale, test_type: TETO_EGEN_TEST_TYPE })
    .select('id')
    .single();
  if (error) {
