...
export const POST = withErrorHandling('personality/slang/start', async (req: Request) => {
  const bypass =
    isRateLimitDisabled() ||
  ...
  if (!isSupabaseConfigured()) {
    ...
    return NextResponse.json({
      ...
      questions: getSlangQuestions(locale),
    });
  }
  ...
  const { data, error } = await admin
    .from('test_sessions')
    .insert({ locale, test_type: SLANG_TEST_TYPE })
    .select('id')
    .single();
  if (error) {
