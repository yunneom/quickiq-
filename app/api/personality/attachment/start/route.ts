...
export const POST = withErrorHandling('personality/attachment/start', async (req: Request) => {
  const bypass =
    isRateLimitDisabled() ||
  ...
  if (!isSupabaseConfigured()) {
    return NextResponse.json({
      ...
      questions: getAttachmentQuestions(locale),
    });
  }
  ...
  const { data, error } = await admin
    .from('test_sessions')
    .insert({ locale, test_type: ATTACHMENT_TEST_TYPE })
    .select('id')
    .single();
  if (error) {
