...
import { scoreAttachment, ATTACHMENT_TEST_TYPE } from '@/lib/personality/attachment';
...
export const POST = withErrorHandling('personality/attachment/submit', async (req: Request) => {
  const bypass =
    isRateLimitDisabled() ||
  ...
  const result = scoreAttachment(body.answers, locale);
  ...
  if (isSupabaseConfigured()) {
    ...
    const { data: updated, error } = await admin
      ...
      .update({
        ...
        test_type: ATTACHMENT_TEST_TYPE,
      })
