...
export const POST = withErrorHandling('personality/tf/start', async (req: Request) => {
  const bypass =
    isRateLimitDisabled() ||
  ...
  if (!isSupabaseConfigured()) {
    ...
    return NextResponse.json({
      ...
      questions: getTfQuestions(locale),
    });
  }
  ...
  const { data, error } = await admin
    .from('test_sessions')
    .insert({ locale, test_type: TF_TEST_TYPE })
    .select('id')
    .single();
  if (error) {
