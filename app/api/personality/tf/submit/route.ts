...
import { scoreTf, TF_TEST_TYPE } from '@/lib/personality/tf';
...
export const POST = withErrorHandling('personality/tf/submit', async (req: Request) => {
  const bypass =
    isRateLimitDisabled() ||
  ...
  const result = scoreTf(body.answers, locale);
