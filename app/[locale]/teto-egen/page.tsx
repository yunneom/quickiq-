...
export const generateMetadata = makeTestLandingMetadata('teto-egen');

const COPY = {
  ko: {
    eyebrow: 'TETO · EGEN',
    headline: '당신은\n테토? 에겐?',
    sub: '15문항 · 2분이면 끝나는 호르몬 성격 유형 테스트.\n4가지 유형 중 당신은 어디?',
    cta: '시작하기 · 무료',
    bullets: [
      '15문항 · 2분',
      '4가지 유형 — 테토남 · 테토녀 · 에겐남 · 에겐녀',
      '연인·친구 궁합 매칭 포함',
      '응시·로그인 정보 없음',
    ],
  ...
  en: {
    eyebrow: 'TETO · EGEN',
    headline: 'Are you\nTeto or Egen?',
    sub: 'A 15-question, 2-minute hormone personality test.\nFind out which of 4 types fits you.',
    cta: 'Start · Free',
    bullets: [
      '15 questions · 2 minutes',
      '4 types — Teto Guy/Girl · Egen Guy/Girl',
...
export default function TetoEgenLanding({
  params: { locale },
}: {
  ...
  const entry = getTestEntry('teto-egen');
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col px-5 pb-10 pt-12">
      ...
      <header className="text-xs font-semibold uppercase tracking-widest text-pink-600">
        {t.eyebrow}
      </header>
        ...
        <ul className="mt-8 space-y-3 text-sm text-gray-700">
          {t.bullets.map((b) => (
            <li key={b} className="flex items-center gap-3">
              <span className="grid h-6 w-6 place-items-center rounded-full bg-pink-100 text-pink-600">
