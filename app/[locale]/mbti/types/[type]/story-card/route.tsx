...
export async function GET(_req: Request, { params }: Params) {
  ...
  const entry = getTestEntry('mbti');
  const profile = getRegistryEntry('mbti')?.getProfile(params.type, loc);
  if (!profile) return new Response('not found', { status: 404 });
  const res = await renderProfileStory({ origin, entry, profile, locale: loc });
  return new Response(res.body, {
    headers: {
      'Content-Type': 'image/png',
      'Content-Disposition': `inline; filename="quickiq-${'mbti'}-${params.type}-story.png"`,
