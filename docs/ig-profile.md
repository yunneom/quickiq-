...
자동 (이미 배포됨):
- **하루 1개 릴스** 자동 발행 — 09:00 KST 크론이 그날의 게시물을 올리고,
  21:00 KST 크론은 실패분 재시도 전용. (2026-09-09부터. 그 전엔 하루 3개였는데
  같은 틀의 릴스가 서로 도달을 갉아먹어 급감했고, 양보다 질로 전환)
...
- 영상 파이프라인이 실패하면 정사각 이미지 포스트로 자동 폴백 —
  일일 발행은 절대 죽지 않음
- 4일 주기: 도형 퍼즐 3일 → "일반" 1일(실제 문항, 실제 문항, 텍스트 베이트
  순으로 교대). 도형 75% / 일반 25% 비율은 하루 3개 시절과 동일하고, 각
  풀은 전환 전에 쓰던 자리에서 이어서 순회하므로 최근에 올린 퍼즐이 바로
  다시 나오지 않음
- 모든 캡션에 IQ 테스트 홍보 블록 + 해시태그 자동 포함
- 정답은 카드/캡션에 절대 노출 안 됨
- **다음날 정답 공개 자동**: 오늘 게시물을 올린 뒤 크론이 최근 3일 중 아직
  공개 안 된 게시물에 "✅ Answer: B — …" + 한 줄 해설 댓글을 답니다. 시드
  댓글의 "answer in 24h" 약속을 코드가 지키는 것이고, 원장에 시각이 찍혀
  한 게시물에 한 번만 달립니다. 테스트 게시물(`?test=`)은 공개하지 않음

수동 (하루 5분, 효과 큼):
1. **당일에는 정답 댓글 금지** — 발행 당일 정답을 직접 달지 말 것. 댓글창이
   토론장이 되는 게 알고리즘 연료입니다. (다음날 공개는 크론이 자동으로 함)
2. 발행 30분~1시간 후 시드 댓글 중 "Answer … in 24h" 줄을 고정(pin).
   핀 고정은 API가 없어 앱에서만 됩니다.
3. 좋아요/댓글에 하트 몇 개 (초기 계정 신뢰 신호).
